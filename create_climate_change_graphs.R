...
res <- dbSendQuery(db, "SELECT posted_date_iso8601 AS datestr, class_cc AS class, count(*) AS count FROM tweets WHERE about_cc == 1 GROUP BY posted_date_iso8601, class_cc")
cc_dates <- fetch(res, n = -1)
cc_dates$date <- as.Date(cc_dates$datestr, "%Y-%m-%d")
# drop datestr
cc_dates <- cc_dates[,2:4]
date_totals <- aggregate(cc_dates$count, by=list(cc_dates$date), FUN=sum)
...
ggplot(cc_dates, aes(x=date, y=count, fill=class)) + 
...
ggsave(filename="cc_dates.pdf", width=9, height=6)

res <- dbSendQuery(db, "SELECT substr(posted_time_iso8601,1,13) AS timestr, class_cc AS class, count(*) AS count FROM tweets WHERE about_cc == 1 AND posted_date_iso8601 IN ('2014-11-10', '2014-11-11') GROUP BY substr(posted_time_iso8601,1,13), class_cc")
cc_day <- fetch(res, n = -1)
cc_day$time <- as.POSIXct(cc_day$timestr, format = "%Y-%m-%dT%H", tz="GMT")
# Lazy fix so that bars are plotted in a better way (add half an hour to each time)
cc_day$time = cc_day$time + 1800
# drop datestr
cc_day <- cc_day[,2:4]
time_totals <- aggregate(cc_day$count, by=list(cc_day$time), FUN=sum)
...
ggplot(cc_day, aes(x=time, y=count, fill=class)) + 
  geom_bar(position="fill", stat="identity") + scale_fill_manual(values=c("gray", "#fc8d62", "#8da0cb", "#000000"), labels=c("NA", "Oppose", "Support", "Scaled Volume")) + 
...
ggsave(filename="cc_day.pdf", width=9, height=6)

res <- dbSendQuery(db, "SELECT mentions.user_mention_username as handle, mentions.user_mention as name, class_cc AS class, count(*) as count FROM tweets INNER JOIN mentions ON tweets.id = mentions.tweet_id WHERE about_cc == 1 AND mentions.user_mention_username IS NOT NULL AND mentions.user_mention_username IN (SELECT username FROM important_handles) GROUP BY mentions.user_mention_username, class_cc")
govt <- fetch(res, n = -1)
...
ggplot(impt_govt_support, aes(x=proportion, y=name)) + 
  ...
  labs(x = "Proportion of Tweets supporting Climate Change", y = "Person mentioned in Tweet") + 
  scale_size("log(Tweets)") +
  theme_classic()
ggsave(filename="cc_govt.pdf", width=10, height=8)

res <- dbSendQuery(db, "SELECT mentions.user_mention_username as handle, 
                   mentions.user_mention as name, 
                   class_cc AS class, count(*) as count 
                   FROM tweets 
                   ...
                   AND mentions.user_mention_username IN (SELECT user_mention_username 
                    ...
                    AND tweet_id IN (SELECT id FROM tweets WHERE about_cc == 1) 
                    GROUP BY user_mention_username 
                    ORDER BY count(*) DESC LIMIT 25
                    ) 
                   GROUP BY mentions.user_mention_username, class_cc")
...
ggplot(mentions_totals_support, aes(x=proportion, y=name)) + 
  geom_point(aes(size=transformed)) + 
  labs(x = "Proportion of Tweets supporting Climate Change (n.b. x-axis limits)", y = "User mentioned in Tweet") + 
  scale_size("log(Tweets)") +
  theme_classic()
ggsave(filename="cc_mentions.pdf", width=10, height=8)
